			...
			for offset <= p.bufLen-cstpHeaderLen {
				...
				if offset <= p.bufLen-3 && p.buf[offset] == 'S' && p.buf[offset+1] == 'T' && p.buf[offset+2] == 'F' {
					...
					if cstpOffset <= p.bufLen-cstpHeaderLen && p.buf[cstpOffset] == 0x01 {
						...
						if offset > 0 || packetType == PacketTypeError || packetLen > 500 {
							...
							log.Printf("OpenConnect: CSTP header bytes at offset %d: %x (type: 0x%02x, length: %d, %s)",
								cstpOffset, headerPreview[:previewLen], packetType, packetLen, endianStr)
						}
				...
				if p.buf[offset] == 0x01 && offset <= p.bufLen-8 {
					...
					if packetLen <= maxPacketSize && totalPacketSize <= maxPacketSize+8 {
						...
						if offset > 0 {
							log.Printf("OpenConnect: Found CSTP header at offset %d (no STF prefix, legacy format), removing %d bytes", offset, offset)
							copy(p.buf, p.buf[offset:])
							p.bufLen -= offset
						}
		...
		if p.state == stateNeedPayload {
			...
			if p.packetType == PacketTypeData && p.bufLen >= payloadStart+20 {
				...
				if p.buf[payloadStart] == 0x45 {
					// IPv4 packet - log ICMP type for debugging
					if p.bufLen >= payloadStart+28 && p.buf[payloadStart+9] == 1 {
						icmpType := p.buf[payloadStart+20]
						icmpTypeStr := "unknown"
						switch icmpType {
						case 0:
						...
						srcIP := net.IP(p.buf[payloadStart+12 : payloadStart+16])
						dstIP := net.IP(p.buf[payloadStart+16 : payloadStart+20])
						log.Printf("OpenConnect: Found IPv4 ICMP %s packet at buffer offset %d: %s -> %s",
							icmpTypeStr, payloadStart, srcIP.String(), dstIP.String())
					} else {
						log.Printf("OpenConnect: Found IPv4 header at buffer offset %d", payloadStart)
					}
				} else if (p.buf[payloadStart] & 0xf0) == 0x60 {
					log.Printf("OpenConnect: Found IPv6 header at buffer offset %d", payloadStart)
				} else {
					...
					for offset := payloadStart; offset < payloadStart+4 && offset < p.bufLen-20; offset++ {
						if p.buf[offset] == 0x45 || (p.buf[offset]&0xf0) == 0x60 {
							...
							if offset != payloadStart {
								log.Printf("OpenConnect: Found IP header at offset %d (expected at %d, possible padding)", offset, payloadStart)
							}
							break
						}
					}
			...
			if actualPacketSize < p.bufLen {
				...
				if leftoverLen > 0 && leftoverLen < 20 {
					...
					log.Printf("OpenConnect: Removing %d bytes, %d bytes will remain in buffer (preview: %x)",
						actualPacketSize, leftoverLen, p.buf[actualPacketSize:actualPacketSize+previewLen])
				}
			}
